! CHECKSIZE defined warns if a sendbuffer is larger than 2GB
!FMM integer size
#define MP_DEFAULTINT kind=8
#define BLUEGENE
!==================================================================
!ARMCI Bindings
!==================================================================
...
contains
    ...
    implicit none
    integer (MP_DEFAULTINT) :: proc,waitcount    
    integer (MyARMCI_Proc) :: proc_tmp
...
!==================================================================
    ...
    implicit none
    integer (MP_DEFAULTINT), target :: src
    ...
    if(sendsize.le.0) then
        msg = "FMM: mp_wrapper: put (MP_DEFAULTINT) sendsize <= 0" // c_null_char 
        call armci_error(msg,ierr)
    endif        
#endif        
    ierr = armci_put(c_loc(src),rptr,sendsize,proc_tmp)
    if (ierr.ne.MyARMCI_SUCCESS) then
        msg = "FMM: ARMCI: put (MP_DEFAULTINT) error" // c_null_char 
        call armci_error(msg,ierr)
    end if
    ...
    implicit none
    integer (MP_DEFAULTINT), target, dimension(*) :: src
    type (c_ptr), target :: rptr
    integer (MP_DEFAULTINT) :: elem,proc
    integer (MyARMCI_Sendsize) :: sendsize
    integer (MyARMCI_Proc) :: proc_tmp    
    integer (MyARMCI_Errorcode) :: ierr
    character (kind=c_char,len=40) :: msg
    ...
    ierr = armci_put(c_loc(src),rptr,sendsize,proc_tmp)
    if (ierr.ne.MyARMCI_SUCCESS) then
        msg = "FMM: ARMCI: put (MP_DEFAULTINT) 1d error" // c_null_char 
        call armci_error(msg,ierr)
    end if
    ...
    implicit none
    integer (MP_DEFAULTINT) :: elem1,elem2,proc
    integer (MP_DEFAULTINT), target, dimension(elem1,elem2) :: src
    type (c_ptr), target :: rptr
    integer (MyARMCI_Sendsize) :: sendsize
    integer (MyARMCI_Proc) :: proc_tmp    
    integer (MyARMCI_Errorcode) :: ierr
    ...
    ierr = armci_put(c_loc(src),rptr,sendsize,proc_tmp)
    if (ierr.ne.MyARMCI_SUCCESS) then
        msg = "FMM: ARMCI: put (MP_DEFAULTINT) 2d error" // c_null_char 
        call armci_error(msg,ierr)
    end if
...
!==================================================================
    ...
    implicit none
    integer (MP_DEFAULTINT) :: lb1,ub1,lb2,ub2,proc
...
implicit none
    ...
    integer(MP_DEFAULTINT) :: attached = 0
...
!==================================================================    
    ...
    implicit none
    real(kind=4), dimension(:,:)    :: dst
    integer(MP_DEFAULTINT)        :: elem,lo,hi
    integer(MyMPI_Op)        :: op
    integer(MyMPI_Comm)         :: comm
    integer(MyMPI_Errorcode)    :: ierr,ierr2
    integer(MyMPI_Entries)        :: elem_tmp
...
!==================================================================        
    ...
    implicit none
    byte, dimension(:,:,:)    :: dst
    integer(MP_DEFAULTINT)    :: elem,lo1,hi1,lo2,hi2
    integer(MyMPI_Comm)     :: comm
    integer(MyMPI_Errorcode):: ierr,ierr2
    integer(MyMPI_Entries)    :: elem_tmp
...
contains
    ...
    integer (4) :: elems
        ...
        if (initialized) then
            write(6,*) "MPI was initialized before DMAPP :-("
            ierr2 = armci_init()
    ...
    implicit none
    type (c_ptr), dimension(*), target :: ptr
    integer(MP_DEFAULTINT) :: bsize,ierr
