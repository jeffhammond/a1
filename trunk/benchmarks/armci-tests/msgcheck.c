...
#define MAXLENG 256*1024
  ...
  for (len=1; len<MAXLENG; len*=2) {
    ...
    for (i=0; i<len; i++){
      if (itest[i] != i) ARMCI_Error("int broadcast failed",i);
                      //armci_die2("int broadcast failed", i,itest[i]);
      if (ltest[i] != (long)i) ARMCI_Error("long broadcast failed",i);
                      //armci_die2("long broadcast failed", i,(int)ltest[i]);
      if (dtest[i] != (double)i) ARMCI_Error("double broadcast failed",i);
