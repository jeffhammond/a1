...
A1D_Send_info_t A1D_Send_info;
A1D_GlobalBarrier_info_t A1D_GlobablBarrier_info;
A1D_Memregion_t **A1D_Memregion_lists;
...
DCMF_Memregion_t *A1D_Fencememory;
...
void A1DI_CtrlRcv_callback (void *clientdata, const DCMF_Control_t *info, size_t peer) {
    ...
    if(A1D_Control_info.exchange_ptr != NULL) {    
       ...
       A1D_Control_info.exchange_ptr[peer] = newregion->vaddress;
    }
 
    --(*((uint32_t *) clientdata));
}

void A1DI_RecvSendShort_callback (void *clientdata, const DCQuad *msginfo, unsigned count, size_t peer,
                             const char *src, size_t bytes) {

     if(A1D_Send_info.exchange_ptr != NULL) 
         memcpy(A1D_Send_info.exchange_ptr + bytes*peer, src, bytes);         
...
DCMF_Result A1DI_Send_initialize()
{
    DCMF_Result result = DCMF_SUCCESS;
    ...
    conf.network = DCMF_TORUS_NETWORK;
    conf.cb_recv_short = A1DI_RecvSendShort_callback;
    conf.cb_recv_short_clientdata = (void *) &A1D_Send_info.rcv_active;
    conf.cb_recv = NULL; 
    conf.cb_recv_clientdata = NULL;
 
    result = DCMF_Send_register(&A1D_Send_info.protocol, &conf);
    A1U_ERR_POP(result,"send registartion returned with error %d \n",result);
...
int A1DI_Fencememory_allocate() {

    DCMF_Result result = DCMF_SUCCESS;
    DCMF_Request_t request[A1D_Process_info.num_ranks];
    DCMF_Callback_t callback;
    char window;     
    int out, snd_active, rank;
    DCQuad msginfo;

    A1U_FUNC_ENTER();

    A1D_Fencememory = (DCMF_Memregion_t *) malloc (sizeof(DCMF_Memregion_t)*A1D_Process_info.num_ranks);
 
    result = DCMF_Memregion_create(&A1D_Fencememory[A1D_Process_info.my_rank], &out, 1, &window, 0);
    A1U_ERR_POP(result =  (result || out < 1),"fence memregion create returned with error \n");

    callback.function = A1DI_Generic_callback;
    callback.clientdata = (void *) &snd_active;

    A1D_Barrier();

    A1D_Send_info.exchange_ptr = A1D_Fencememory;
    A1D_Send_info.rcv_active = A1D_Process_info.num_ranks-1;
    snd_active = A1D_Process_info.num_ranks-1;
    for(rank=0; rank<A1D_Process_info.num_ranks; rank++) {
        if(rank != A1D_Process_info.my_rank) {
            DCMF_Send(&A1D_Send_info.protocol, 
                      &request[rank],
                      callback, 
                      DCMF_SEQUENTIAL_CONSISTENCY,
                      rank,  
                      sizeof(DCMF_Memregion_t),
                      (char *) &A1D_Fencememory[A1D_Process_info.my_rank],
                      &msginfo, 
                      1);
        }
    }
    while(A1D_Send_info.rcv_active || snd_active) A1DI_CRITICAL(DCMF_Messager_advance());     
    A1D_Send_info.exchange_ptr = NULL;    
...
int A1D_Initialize(int thread_level, int num_threads,
    ...
    result = A1DI_Send_initialize();
    A1U_ERR_POP(result,"Send initialize returned with error \n");
    ...
    result = A1DI_Fencememory_allocate(); 
    A1U_ERR_POP(result,"Request initialization failed \n");    
