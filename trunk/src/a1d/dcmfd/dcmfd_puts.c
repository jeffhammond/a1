        ...
        int *count, int stride_level)
{
    ...
    if(stride_level > 0) {
         ...
         {
            pointer = A1DI_Pack_data(pointer, source_ptr+i*src_stride_ar[stride_level], src_stride_ar,\
        ...
        int *trg_stride_ar, int *count, int stride_levels)
{
    ...
    *size_packet = sizeof(A1D_Pack_header_t) + size_data; 
    ...
    memcpy(header.src_stride_ar, src_stride_ar, stride_levels*sizeof(int));
    memcpy(header.trg_stride_ar, trg_stride_ar, stride_levels*sizeof(int));
