...
int A1DI_Packed_gets(int target, void* source_ptr, int *src_stride_ar, void* target_ptr,\
...
int A1DI_Direct_gets(int target, void* source_ptr, int *src_stride_ar, void* target_ptr,\
        int *trg_stride_ar, int* count, int stride_level)  
{
    ...
    if(stride_level > 0) {
         ...
         {
            A1DI_Direct_gets(target, (void *) ((size_t)source_ptr + i*src_stride_ar[stride_level-1]), src_stride_ar,\
         ...
         int *trg_stride_ar, int *count, int stride_levels) 
{
    ...
    if(count[0] >= a1_direct_noncontig_threshold) {
      ...
      result = A1DI_Direct_gets(target, source_ptr, src_stride_ar, target_ptr,
      ...
      result = A1DI_Packed_gets(target, source_ptr, src_stride_ar, target_ptr,
          trg_stride_ar, count, stride_levels);
